...
const Event = ({ img, title, imgs, point }: IEvent) => {
  ...
  return (
    <>
      <div
        className="flex flex-col max-w-[1440px] md:w-[80%] w-[90%] h-[250px] mx-auto"
      ...
      >
        <div className="relative">
          <Image
            ...
            height={220}
            alt="event-img"
            className="w-[210px] h-[210px] rounded-md object-cover mb-3"
          />
        ...
        <span className="text-center h-[50px] font-poppins text-[14px] font-semibold">
          {title}
        </span>
      </div>
      {showDetails && point === current && (
        <DetailsModal
          images={imgs}
          open={showDetails}
          close={() => setShowDetails(false)}
          title={`${title} In A Glance`}
        />
      )}
    </>
  );
...
const TopEvents = () => {
  ...
  return (
    <div id="events" className="flex pb-8 flex-col">
      <h2 className=" text-[1.5rem] md:text-[3rem] text-center font-semibold py-10">
        Top Events
      </h2>
      ...
      <div className="bg-image-with-overlay  study-bg-imgs p-10 relative h-[400px]">
        <Image
          src={"/img/president.jpeg"}
          alt="pastor_image"
          className="absolute rounded-t-xl -bottom-3"
          width={320}
          height={300}
        />
      </div>
      <div className="flex flex-col p-10  items-center justify-center">
        <span className="font-semibold font-gara text-[16px] pb-2">
        ...
        <button
          ...
          className="w-[270px] mt-8 bg-gray-700  border-none text-white py-4 font-semibold text-[18px]"
        >
          Register Here
        </button>
      </div>
